...
const User = {
  ...
  crear: async (
    connection,
    { id_persona, email, password, role_id, username }
  ) => {
    const [result] = await connection.query(
      `INSERT INTO users (id_persona, email, password, role_id, username)
       VALUES (?, ?, ?, ?, ?)`,
      [id_persona, email, password, role_id, username]
    );
  ...
  updatePassword: async (id, connection, newPassword) => {
    ...
    await connection.query(
      "UPDATE users SET password = ?, change_password_required = ? WHERE id = ?",
